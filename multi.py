...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, simulation, lane, vehicleClass, direction_number, direction):
        ...
        if len(vehicles_in_lane) > 1:
            prev_vehicle = vehicles_in_lane[-2] 
            if direction == 'right': self.stop = prev_vehicle.stop - prev_vehicle.image.get_rect().width - self.simulation.stoppingGap
            elif direction == 'left': self.stop = prev_vehicle.stop + prev_vehicle.image.get_rect().width + self.simulation.stoppingGap
            elif direction == 'down': self.stop = prev_vehicle.stop - prev_vehicle.image.get_rect().height - self.simulation.stoppingGap
            elif direction == 'up': self.stop = prev_vehicle.stop + prev_vehicle.image.get_rect().height + self.simulation.stoppingGap
        else:
    ...
    def update(self):
        ...
        can_move_based_on_light = self.crossed == 1 or is_light_green
        ...
        if can_move_based_on_light and gap_is_clear:
        ...
        if self.crossed == 0 and is_light_green:
        ...
        if (self.x > 1400 or self.x < -200 or self.y > 800 or self.y < -200):
            vehicles_in_lane.remove(self)
            self.kill()

        self.rect.topleft = (self.x, self.y)

# MODIFIED FUNCTION to draw the new detailed dashboard
