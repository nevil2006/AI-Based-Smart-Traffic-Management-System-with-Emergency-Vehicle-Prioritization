...
HMAC_SECRET = os.environ.get("HMAC_SECRET", "demo_secret_key_change_me").encode()


def canonical_json(data: dict) -> str:
    """Stable JSON serialization so signature is consistent."""
    return json.dumps(data, sort_keys=True, separators=(",", ":"))


def sign_packet(packet: dict) -> str:
    """Return hex HMAC SHA256 signature."""
    msg = canonical_json(packet).encode()
    return hmac.new(HMAC_SECRET, msg, hashlib.sha256).hexdigest()


def verify_packet(packet: dict, signature: str) -> bool:
    """Verify signature."""
    expected = sign_packet(packet)
    return hmac.compare_digest(expected, signature)


LAST_ACCEPTED_CYCLE_ID = -1
...
while process_running:
    ...
    control_packet = {
        "junction_id": "TN_JNC_01",
        "cycle_id": int(current_time),  
        "timestamp": int(current_time),
        "active_lane": f"Lane_{active_lane_idx+1}",
        "lane_timings": lane_timings_json
    }

    signature = sign_packet(control_packet)
    ...
    print(f"\nActive Lane: Lane_{active_lane_idx + 1}")

    print("\n--- SECURE SIGNAL COMMAND ---")
    print(json.dumps(secured_output, indent=4))

    receiver_packet = secured_output["packet"]
    receiver_sig = secured_output["hmac_sha256"]

    if receiver_packet["cycle_id"] <= LAST_ACCEPTED_CYCLE_ID:
        print(" PLC VERIFY: REPLAY ATTACK DETECTED - Old cycle rejected")
    else:
        if verify_packet(receiver_packet, receiver_sig):
            LAST_ACCEPTED_CYCLE_ID = receiver_packet["cycle_id"]
            print(" PLC VERIFY: SIGNATURE VALID - Command Accepted")
        else:
            print(" PLC VERIFY: SIGNATURE INVALID - Command Rejected")

    tampered = json.loads(json.dumps(receiver_packet))  
    tampered["lane_timings"]["Lane_1"]["Green_Time_Seconds"] += 50  

    if verify_packet(tampered, receiver_sig):
        print(" ATTACK TEST: Tampering NOT detected (bad)")
    else:
        print(" ATTACK TEST: Tampering detected and rejected")
