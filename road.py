...
class SimulationInstance:
    def __init__(self, scenario_name, scenario_dist):
        ...
        # Y-coordinates adjusted for better horizontal alignment
    ...
    def draw_road_borders(self, surface):
        """Draws thick red borders on the approaching roads."""
        RED = (255, 0, 0)
        LINE_WIDTH = 5

        border_lines = [
            # Approaching from LEFT (direction 'right')
            [(0, 342), (550, 342)],  # Top border
            [(0, 432), (550, 432)],  # Bottom border

            # Approaching from TOP (direction 'down')
            [(692, 0), (692, 290)],  # Left border
            [(782, 0), (782, 290)],  # Right border

            # Approaching from RIGHT (direction 'left')
            [(1400, 442), (840, 442)], # Top border
            [(1400, 532), (840, 532)], # Bottom border

            # Approaching from BOTTOM (direction 'up')
            [(607, 800), (607, 575)], # Left border
            [(697, 800), (697, 575)]  # Right border
        ]

        for points in border_lines:
            pygame.draw.lines(surface, RED, False, points, LINE_WIDTH)
    
    def draw(self, surface, assets):
        surface.blit(assets['background'], (0, 0))
        
        self.draw_road_borders(surface)
